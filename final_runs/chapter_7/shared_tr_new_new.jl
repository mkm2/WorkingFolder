...
if MULT_RANDOM_STATES == false
    ...
    Threads.@threads for shot in 1:SHOTS
        ...
        @time fidelities[:,shot] = fidelity_timereversal(H_tot[shot],trange,ψ0,SEQ_NAME,SEQ_REPS,N,rotations,proto_hamiltonians,tmax)
        @time oto_commutators[:,:,shot] = oto_commutator_timereversal(H_tot[shot],A,B,trange,ψ0,SEQ_NAME,SEQ_REPS,N,rotations,proto_hamiltonians,tmax)
    ...
    for shot in 1:SHOTS
        ...
        Threads.@threads for s in 1:N_RANDOM_STATES
            @time fidelities[:,shot,s] = fidelity_timereversal(H_total,trange,ψs[:,s],SEQ_NAME,SEQ_REPS,N,rotations,proto_hamiltonians,tmax)
            @time oto_commutators[:,:,shot,s] = oto_commutator_timereversal(H_total,A,B,trange,ψs[:,s],SEQ_NAME,SEQ_REPS,N,rotations,proto_hamiltonians,tmax)
