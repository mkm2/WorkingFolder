...
# ╔═╡ 024e0eb0-59f4-454c-9b32-588f0d414349
using SparseArrays, LinearAlgebra

# ╔═╡ 17b05d7a-beb9-49a8-8b79-0e83479015d2
using Random, Statistics

# ╔═╡ 0bd4eaf9-9289-4a17-8654-787f96365c1a
using RandomMatrices,QuantumInformation

# ╔═╡ af04c2f2-7eca-11ec-364c-95962778815b
md"# Quantum Typicality - Sampling from the Haar measure"

# ╔═╡ e1e39775-ac38-40e7-9b3c-1894246d755e
begin
	n = 1000
	N = 23
	k = 2^N
	d = Haar(2) #Circular Unitary Ensemble
	c = CircularEnsemble{2}(k)
	ψ0 = normalize!(randn(k))
end

# ╔═╡ b8deed83-c2e7-40e1-81aa-543c057d4bf9
@time randn(k)+im*randn(k)

# ╔═╡ f27d5675-7c68-4bba-bf78-efffeb012c72
@time randn(ComplexF64,k)

# ╔═╡ 59a7750a-4e61-470f-a3e0-cefe504d726f
begin
	#rand_ψ_norm = zeros(ComplexF64,n,k)
	for i = 1:n
	    v = randn(k) + im *randn(k)
		renormalize!(v)
		v
	    #rand_ψ_norm[i,:] = v / norm(v)  # normalize to unit norm
	end
end

# ╔═╡ 6ac1fc48-8fda-4b2d-9588-26d54de36690
begin
	#rand_ψ_norm2 = zeros(ComplexF64,n,k)
	for i = 1:n
	    v = randn(ComplexF64,k)
		renormalize!(v)
		v
	    #rand_ψ_norm2[i,:] = v / norm(v)  # normalize to unit norm
	end
end

# ╔═╡ 0adcf9fc-72f4-4197-885e-fb09f9064811
@time rand(d,k)

# ╔═╡ 83e08f2b-6c67-4194-ac85-6a0572fdc106
begin
	#Random matrices
	rand_ψ_RM = zeros(ComplexF64,n,k)
	for i = 1:n
		R = rand(d, k)
		rand_ψ_RM[i,:] = R*ψ0
	end
...
# ╔═╡ 4eaf8014-18c7-44fc-bc60-e66e58355a38
@time rand(c)

# ╔═╡ 4031719a-0b37-43be-90eb-442827406460
#QuantumInformation
begin
	rand_ψ_QI = zeros(ComplexF64,n,k)
	for i = 1:n
		U = rand(c)
		rand_ψ_QI[i,:] = U*ψ0
	end
